<template>
  <div>
    <a-card style="width: 100%" :tab-list="types" :active-tab-key="currentType" @tabChange="key => onTabChange(key)">
      <p v-show="currentType === 'second'">
        <crontab-second @update="updateCrontabValue" :check="checkNumber" ref="cronsecond" />
      </p>
      <p v-show="currentType === 'min'">
        <crontab-min @update="updateCrontabValue" :check="checkNumber" ref="cronmin" />
      </p>
      <p v-show="currentType === 'hour'">
        <crontab-hour @update="updateCrontabValue" :check="checkNumber" ref="cronhour" />
      </p>
      <p v-show="currentType === 'day'">
        <crontab-day @update="updateCrontabValue" :check="checkNumber" ref="cronday" />
      </p>
      <p v-show="currentType === 'month'">
        <crontab-month @update="updateCrontabValue" :check="checkNumber" ref="cronmonth" />
      </p>
      <p v-show="currentType === 'week'">
        <crontab-week @update="updateCrontabValue" :check="checkNumber" ref="cronweek" />
      </p>
      <p v-show="currentType === 'year'">
        <crontab-year @update="updateCrontabValue" :check="checkNumber" ref="cronyear" />
      </p>
    </a-card>
    <a-card style="width: 100%">
      <a-divider orientation="left"> 表达式字段 </a-divider>
      <a-row :gutter="16">
        <a-col :md="3" :sm="8">
...
export default {
  ...
  methods: {
    ...
    changeRadio(name, value) {
      ...
      if (arr.includes(name)) {
        if (value === '*') {
          ...
          isNaN(indexArr[0]) ? (this.$refs[refName].cycle01 = 0) : (this.$refs[refName].cycle01 = Number(indexArr[0]))
        ...
        if (value === '*') {
          ...
          isNaN(indexArr[0]) ? (this.$refs[refName].workday = 0) : (this.$refs[refName].workday = Number(indexArr[0]))
          insValue = 5
        } else if (value === 'L') {
          insValue = 6
        } else {
        ...
        if (value === '*') {
          ...
          isNaN(indexArr[0]) ? (this.$refs[refName].weekday = 1) : (this.$refs[refName].weekday = Number(indexArr[0]))
          insValue = 5
        } else {
          this.$refs[refName].checkboxList = value.split(',')
          insValue = 6
    ...
    doCopy() {
      this.$copyText(this.crontabValueString)
        .then(message => {
