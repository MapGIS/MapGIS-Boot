  ...
  <div>
    <a-space>
      <a-dropdown v-if="!sizeHide" :trigger="['click']">
        <a-button type="dashed" shape="circle" icon="column-height" />
      ...
      <a-popover v-if="!settingHide" placement="bottomRight" trigger="click">
        ...
        <template slot="title">
          <a-checkbox :indeterminate="indeterminate" :checked="checkAll" @change="onCheckAllChange">
            列展示/排序
            <a-tooltip>
              <template slot="title"> 列排序需拖动名称 </template>
...
export default {
  ...
  watch: {
    checkedKeys(cur, last) {
      const checked = cur.filter(function (v) {
        return last.indexOf(v) === -1
      })
      const canceled = last.filter(function (v) {
