...
set OUTPUT_DIR=%ROOT_DIR_REV%/../output-loongarch64
if "%~3" neq "" set "OUTPUT_DIR=%~3"

if "%COMPILE_FORNT_BACK%" == "true" (
    echo. | call %BASE_PACKER_DIR%/build.bat "local,longxin" %SKIP_FRONT%
) else (
    echo 已跳过前端和后端编译
)
...
rem 打包成发行包
echo. | call %ROOT_DIR_REV%/package-common.bat linux-loongarch64 "%OUTPUT_DIR%"
